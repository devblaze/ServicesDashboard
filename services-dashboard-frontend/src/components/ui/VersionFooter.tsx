...
export const VersionFooter: React.FC<VersionFooterProps> = ({ darkMode = true }) => {
  const { data: versionInfo, isLoading, isError } = useQuery({
  ...
  const version = versionInfo?.version || (versionInfo as any)?.Version;

  const displayVersion = isLoading
    ? '...'
    : version || (isError ? 'error' : 'dev');
  ...
  return (
    <div className={`fixed bottom-4 left-4 px-3 py-1.5 rounded-lg text-xs font-mono backdrop-blur-sm border ${
      ...
      <span className="opacity-75">v</span>{displayVersion}
    </div>
  );
};
