...
{
    Task<IEnumerable<RemoteContainerResult>> ListContainersAsync(string serverId);
    ...
    Task<ContainerStatsResult> GetContainerStatsAsync(string serverId, string containerId);
...
public class RemoteDockerLogCollector : IRemoteLogCollector
{
    ...
    public async Task<IEnumerable<RemoteContainerResult>> ListContainersAsync(string serverId)
    {
        ...
        try
        {
            ...
            foreach (var line in result.Split('\n', StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (parts.Length >= 4)
                {
                    containers.Add(new RemoteContainerResult
                    {
                        Id = parts[0],
                        Name = parts[1],
                        Status = parts[2],
    ...
    public async Task<ContainerStatsResult> GetContainerStatsAsync(string serverId, string containerId)
    {
        var connection = await _connectionManager.GetConnectionByIdAsync(serverId);
        if (connection == null)
        {
